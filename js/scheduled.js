...
window.addEventListener("DOMContentLoaded", () => {
    ...
    closeModalBtn?.addEventListener("click", () => rxModal.style.display = "none");
    ...
    function renderGroupedTable(data) {
        ...
        const headers = ["Name", "Phone", "Address", "RX Details", "Signature", "Actions"];
            ...
            groupedEntries[day].forEach(entry => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        ...
                        <button data-key="${entry.key}" class="deleteBtn" ${entry.signature ? 'disabled' : ''}>Delete</button>
                         <button data-key="${entry.key}" class="editBtn" ${entry.signature ? 'disabled' : ''}>Edit</button>
    ...
    downloadCsvBtn?.addEventListener("click", () => {
        onValue(deliveriesRef, snap => {
            const data = snap.val() || {};
            const now = Date.now();
            const weekAgo = now - 7 * 24 * 60 * 60 * 1000;
            const rows = [["Name", "Phone", "Address", "Date", "RX Details"]];
            Object.values(data).forEach(ent => {
                if (ent.timestamp >= weekAgo) {
                    rows.push([ent.name, ent.phone, ent.address, ent.date, ent.rxDetails]);
                }
            });
